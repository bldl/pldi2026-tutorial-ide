...
import { createBeanServices, BeanLanguageMetaData } from 'bean-language';
...
export const generateAction = async (source: string, destination: string): Promise<void> => {
    const services = createBeanServices(NodeFileSystem).Bean;
    const model = await extractAstNode<Body>(source, services);
...
export default function(): void {
    ...
    const fileExtensions = BeanLanguageMetaData.fileExtensions.join(', ');
    program
        .command('generate')
